							...
							{projects.map(
								({ id, title, subtitle, src, describeJp, code }) => (
									<motion.div
										...
										key={id}
									>
										<div className='absolute h-full w-full opacity-0 hover:opacity-90 transition duration-500 dark:bg-whitish dark:text-bluish-black bg-bluish-black text-whitish z-30 flex flex-col justify-center items-center text-center p-16'>
											<p className='text-3xl font-playfair font-semibold'>
												{title}
											</p>
											<p className='text-xl font-playfair font-semibold mt-2'>
												{subtitle}
											</p>
											<p className='mt-4 font-jp'>{describeJp}</p>
											<a
												href={code}
												className='opacity-80 mt-4 hover:scale-150'
												target='_blank'
												rel='noreferrer'
											>
												<FaGithub size={30} />
							...
							{projects.map(
								({ id, title, subtitle, src, describeEn, code }) => (
									<motion.div
										...
										key={id}
									>
										<div className='absolute h-full w-full opacity-0 hover:opacity-90 transition duration-500 dark:bg-whitish dark:text-bluish-black bg-bluish-black text-whitish z-30 flex flex-col justify-center items-center text-center p-16 text-deep-blue'>
											<p className='text-3xl font-playfair font-semibold'>
												{title}
											</p>
											<p className='text-xl font-playfair font-semibold mt-2'>
											...
											<p className='mt-4 font-opensans'>{describeEn}</p>
											<a
												href={code}
												className='opacity-80 mt-4 hover:scale-150'
												target='_blank'
